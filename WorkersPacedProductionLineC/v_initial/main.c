...
static gboolean draw_cb(GtkWidget *widget, cairo_t *cr, Worker *** workers)
{
	...
	for (i = 0; i < _nbWorkers; i++)
	{
		...
		{
			int FinTache = (*workers)[i]->doneTasks[j]->endingTime;
...
int CommonHeuristicProcess(Station** stations, Task** tasks, float timeBound, int (*selectionFunction)(Task**, int, Worker**), int (*behindDueFunction)(Task**, int **, int),int gtk)
{
	...
	if (gtk == 1)
	{
		...
		//gtk_container_add(GTK_WINDOW(fenetre), dessin);
			...
			for (i = 0; i<nbWorkers; i++)
			{
				if (workers[i]->boolCriticalPath == 0)
				{
					for ( k = 0; k < workers[i]->nbTasks; k++)
					{
						int j = 0;
						int trouve = 0;
						int idTask = workers[i]->doneTasks[k]->id;
...
Worker** HeuristicCalculation(Station** stations, Task** tasks, Task** G, Task** Nplus, Worker** workers, int GSize, int NplusSize, int (*selectionFunction)(Task**, int, Worker**))
{
	...
	int NbWorkersAssigned = 0;
	int SetAllTasks = NplusSize;
	int CompteurTasksToDo = 0;
	float beginningTime = -1.0;
	int * tableauWorkers;
