...
int CommonHeuristicProcess(Station** stations, Task** tasks, float timeBound, int (*selectionFunction)(Task**, int, Worker**), int (*behindDueFunction)(Task**, int ** TasksCriticalPathWay, int nbCriticalPathWay), int gtk);				// common process to calculate an heuristic and printing the results (parameters are described below)
Worker** Heuristic(Station** stations, Task** tasks, float timeBound, int (*selectionFunction)(Task**, int, Worker**), int (*behindDueFunction)(Task**, int ** TasksCriticalPathWay, int nbCriticalPathWay));						// calculate an heuristic; stations: array of stations, tasks: array of tasks, timeBound: due date, selectionFunction: a function to find the task to process, behindDueFunction: a function to change tasks parameter when failing to find a solution; return an array of workers containing who is working where and when
...
void WriteSolution(Worker** workers);	// write the solution found on screen; workers: array of assigned workers
